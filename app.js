	...
	comicsData.forEach(item => {
		...
		if (item.price) {
			if (isCasita(item.url)) {
				...
				priceText = discounted != null ? `<span class="old-price" style="text-decoration: line-through; opacity: 0.75; margin-right: 6px;">${item.price}</span><span class="new-price">${formatPrice(discounted)}</span>` : item.price;
			} else {
